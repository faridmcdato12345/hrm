...
@section('content')
                    ...
                    @foreach($employees as $employee)
                        ...
                        <tr>
                            ...
                            <td class="row">
                                    ...
                                    @if (Auth::user()->designation == 'admin' || Auth::user()->designation == 'CEO')
                                    &nbsp
                            ...
                            <td>
                                @if($employee->leave_status == '' || strtolower($employee->leave_status) == 'Pending on Supervisor')
                                    ...
                                    @endif
                                @endif
                                @if(strtolower($employee->leave_status) == 'pending on ogm')
                                    @if(Auth::user()->id == 1 || (Auth::user()->id != $employee->id))
                                        <select class="update_status form-control" id="{{$employee->leave_id}}" style="width:160px;">
                                            <option value="">Update Status</option>
                                            <option value="Approved">Approved</option>
                                ...
                                @if(strtolower($employee->leave_status) == 'approved' || strtolower($employee->leave_status) == 'declined')
                                    @if ($employee->dm_flag == 1 && $employee->ogm_flag == 0)
                                    {{$employee->leave_status}}<span> by Supervisor</span>       
                                    @endif
                                    @if($employee->ogm_flag == 1 && $employee->dm_flag == 1)
                                    {{$employee->leave_status}}<span> at the OGM</span> 
                                    @endif
                                @endif
                            </td>
                        </tr>
    ...
    <script type="text/javascript">
        ...
        $('.approve_button').on('click',function(){
            location.href = "{{url('/')}}/leave/updateStatus/" + $(this).attr('id') + '/Approved';
        });
        $('.decline_button').on('click',function(){
            location.href = "{{url('/')}}/leave/updateStatus/" + $(this).attr('id') + '/Declined';
        });
    </script>
    {{-- <script>
        $(document).ready(function() {
